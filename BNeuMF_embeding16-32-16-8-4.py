...
import sys
import BXGMF1_3_embeding16, BXMLP1_3_embeding32_16_8_4
import argparse


#################### Arguments ####################
def parse_args():
    parser = argparse.ArgumentParser(description="BNeuMF_embeding16-32-16-8-4.")
    ...
    parser.add_argument('--mf_pretrain', nargs='?', default='Pretrain/ml-1m_BXGMF1_3_embeding16.h5',
...
if __name__ == '__main__':
    f = open("C:/Users/19678/Desktop/BNeuMF_embeding16-32-16-8-4", "w+")
    args = parse_args()
    ...
    print("BNeuMF_embeding16-32-16-8-4 arguments: %s " % (args))
    print >> f, "BNeuMF_embeding16-32-16-8-4 arguments: %s " % (args)
    ...
    time() - t1, num_users, num_items, len(testRatings))
    # Build model
    # (num_users, num_items, mf_dim=10, layers=[10], reg_layers=[0], reg_mf=0)
    ...
    if mf_pretrain != '' and mlp_pretrain != '':
        gmf_model = BXGMF1_3_embeding16.get_model(num_users, num_items, mf_dim)
        gmf_model.load_weights(mf_pretrain)
        mlp_model = BXMLP1_3_embeding32_16_8_4.get_model(num_users, num_items, layers, reg_layers)
        mlp_model.load_weights(mlp_pretrain)
        model = load_pretrain_model(model, gmf_model, mlp_model, len(layers))
        print("Load pretrained BXGMF1_3_embeding16 (%s) and BXMLP1_3_embeding32_16_8_4 (%s) models done. " % (mf_pretrain, mlp_pretrain))
        print >> f, "Load pretrained BXGMF1_3_embeding16 (%s) and BXMLP1_3_embeding32_16_8_4 (%s) models done. " % (mf_pretrain, mlp_pretrain)
    # Init performance
    (hits, ndcgs) = evaluate_model(model, testRatings, testNegatives, topK, evaluation_threads)
    hr, ndcg = np.array(hits).mean(), np.array(ndcgs).mean()
    print('Init: HR = %.4f, NDCG = %.4f' % (hr, ndcg))
    print >> f, 'Init: HR = %.4f, NDCG = %.4f' % (hr, ndcg)
    ...
    #     print >> f, "The best NeuMF model is saved to %s" % (model_out_file)
